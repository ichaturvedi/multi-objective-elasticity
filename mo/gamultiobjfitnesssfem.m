function gamultiobjfitnessfem(inputfile, labelfile)%, outputfile)
...
%FitnessFunction = @(x) fem_multiobjective(x,mv);
...
options = optimoptions(@gamultiobj,'PlotFcn',{@gaplotpareto,@gaplotscorediversity},'UseParallel',true,'CrossoverFraction',0.3,'MaxGenerations',500,'PopulationSize',50)%,'InitialPopulationMatrix',population);
[x2,fval,exitFlag,output,population,scores]=gamultiobj(FitnessFunction,numberOfVariables,A,b,Aeq,beq,lb,ub,options);

save moouts.mat x2 fval population scores output;
